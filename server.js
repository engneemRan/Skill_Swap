...
const { sendContactMessage, sendTestimonial, sendRegistrationEmail } = require('./services/mailService');
...
app.put('/api/users/:id/admin-update', async (req, res) => {
  ...
  try {
    await User.findByIdAndUpdate(req.params.id, { name, email, bio, offers, wants });
    res.json({ success: true });
  } catch {
